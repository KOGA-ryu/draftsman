...
- `ui_theme.*`, `theme_settings_page.*`: theme mode, low-light palette, font settings, persistence, derived UI tokens, and human theme controls.
...
- Do not hard-code one-off UI colors; add a derived token in `ui_rules` when a new surface needs color.
